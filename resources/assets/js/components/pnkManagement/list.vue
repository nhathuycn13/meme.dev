                            ...
                            <div class="table-responsive mailbox-messages">
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tr v-for="item in items">
                                        ...
                                        <td><label :class="item.status.label">{{ item.status.name }}</label></td>
                                        <td>{{ item.user.name }}</td>
                                        <td><timeago :since="item.created_at" :auto-update="60"></timeago></td>
                                        <td>
                                            <div class="btn-group">
    ...
    export default{
        ...
        methods : {
            fetchData : function (page) {
                this.$Progress.start();
                this.$http.get('api/nk', { params : { page : page, perpage : this.data.per_page}}).then(function (response) {
            ...
            deleteMulti : function (id) {
                ...
                if (!confirm) return;
                this.$Progress.start();
                this.$http.delete('api/nk/0', {params : {ids : this.selected}}).then(function (response) {
                    if (response.body > 0){
            ...
            search : function () {
                this.$Progress.start();
                this.$http.post('api/nk/search', { keyword : this.keyword}).then(function (response) {
