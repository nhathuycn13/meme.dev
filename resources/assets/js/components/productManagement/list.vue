                            ...
                            <div class="table-responsive mailbox-messages">
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tr v-for="item in items">
                                        ...
                                        <td class="text-center"><router-link :to="{ name:'view', params : { id : item.id} }">P{{ getId(item.id) }}</router-link></td>
                                        <td><router-link :to="{ name:'view', params : { id : item.id} }">P{{ item.name }}</router-link></td>
                                        <td>{{item.code}}</td>
                                        <!--<td>{{item.type.name ? item.type.name = ''}}</td>-->
                                        <td>
                                            <div class="btn-group">
                                                <router-link :to="{ name: 'view', params: { id : item.id} }"><i class="fa fa-eye" aria-hidden="true"></i></router-link>
    ...
    export default{
        ...
        methods : {
            fetchData : function (page) {
                this.$Progress.start();
                this.$http.get('api/product', { params : { page : page, perpage : this.data.per_page}}).then(function (response) {
            ...
            deleteItem : function (id) {
                ...
                if (!confirm) return;
                this.$Progress.start();
                this.$http.delete('api/product/' + id).then(function (response) {
                    if (response.body == '1'){
            ...
            search : function () {
                this.$Progress.start();
                this.$http.post('api/product/search', { keyword : this.keyword}).then(function (response) {
