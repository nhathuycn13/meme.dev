                            ...
                            <div class="table-responsive mailbox-messages">
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tr v-for="(item, index) in items">
                                        ...
                                        <td>{{ index + 1 }}</td>
                                        <td><router-link :to="{ name:'view', params : { id : item.id} }">{{ item.product.name }}</router-link></td>
                                        <td>{{ item.qty }}</td>
                                        <td>{{ item.price }}</td>
    ...
    export default{
        ...
        methods : {
            fetchData : function (page) {
                this.$Progress.start();
                this.$http.get('api/warehouse', { params : { page : page, perpage : this.data.per_page}}).then(function (response) {
            ...
            deleteItem : function (id) {
                ...
                if (!confirm) return;
                this.$Progress.start();
                this.$http.delete('api/warehouse/' + id).then(function (response) {
                    if (response.body == '1'){
            ...
            deleteMulti : function (id) {
                ...
                if (!confirm) return;
                this.$Progress.start();
                this.$http.delete('api/warehouse/0', {params : {ids : this.selected}}).then(function (response) {
                    if (response.body > 0){
